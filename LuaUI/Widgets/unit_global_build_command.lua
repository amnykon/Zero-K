...
function widget:CommandNotify(id, params, options, isZkMex, isAreaMex)
	if id == CMD_GLOBAL_BUILD then
		SetGlobalBuildState(params and params[1] == 1)
		return true
	end
				...
				if newBuilders[unitID] then -- for new units that have just been added and finished following their constructor separator orders.
					...
					if wasBusyJob then -- if the worker was also still on our busy list
						if areaCmdList[wasBusyJob] then -- if it was an area command
							...
							StopAnyWorker(wasBusyJob) -- remove the job from the queue, since UnitIdle is the only way to tell completeness for area jobs.
						end
					end
				end
			end
...
function RemoveJobs(x, z, r)
	...
	for key, cmd in pairs(buildQueue) do
		...
		if cmd.id < 0 then -- for build jobs
			...
			local dist1 = DistanceSq(x, z, cx-xSize, cz-zSize)
			local dist2 = DistanceSq(x, z, cx+xSize, cz-zSize)
			local dist3 = DistanceSq(x, z, cx-xSize, cz+zSize)
